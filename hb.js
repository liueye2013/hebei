...
// 配置
const CONFIG = {
    LIVE_KEY: "k5m9p2x8r4b3",
    TK_CACHE_DURATION: 10 * 60 * 1000,
    BASE_URL: "https://tv.pull.hebtv.com"
};
...
async function updateCache(channel) {
    ...
    if (cache && cache.t && cache.k && (now - cache.timestamp < CONFIG.TK_CACHE_DURATION)) {
        return cache;
    }

    const t = getBeijingTimestamp();
    const k = md5(realPath + CONFIG.LIVE_KEY + t);
...
// 构建目标URL
function buildTargetUrl(channel, t, k) {
    const realPath = CHANNELS[channel];
    return `${CONFIG.BASE_URL}${realPath}?t=${t}&k=${k}`;
}
